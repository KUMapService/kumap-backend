...
def insert_region_coordinates(connection: object):
    ...
    """
        with open(
            os.path.join(BASE_DIR, "data/PnuCode.csv"), "r", encoding="utf-8"
        ) as rcf:
            lines = rcf.readlines()
        count = 0
        for line in lines:
...
def insert_cadastral_data(connection: object):
  ...
  """
    for scale in ["emd", "sig", "sido"]:
        with open(
            os.path.join(BASE_DIR, "src/dataset/cadastralmap", scale + ".json"), "r"
        ) as gf:
            data = json.load(gf)
        ...
        for polygon in polygons:
            ...
            try:
                ...
                if scale == "emd":
                    print(
                        f"({count:4d}/{len(polygon):4d}) {polygon['properties']['EMD_CD']}"
                    )
                    cursor.execute(
                        query,
                        (
                            polygon["properties"]["EMD_CD"],
                            centroid[1],
                            centroid[0],
                            json.dumps(polygon["geometry"]["coordinates"]),
                        ),
                    )
                    connection.commit()
                    count += 1
                    ...
                    print(
                        f"({count:4d}/{len(polygon):4d}) {polygon['properties']['SIG_CD']}"
                    )
                    cursor.execute(
                        query,
                        (
                            polygon["properties"]["SIG_CD"],
                            centroid[1],
                            centroid[0],
                            json.dumps(polygon["geometry"]["coordinates"]),
                        ),
                    ...
                    print(
                        f"({count:4d}/{len(polygon):4d}) {polygon['properties']['CTPRVN_CD']}"
                    )
                    cursor.execute(
                        query,
                        (
                            polygon["properties"]["CTPRVN_CD"],
                            centroid[1],
                            centroid[0],
                            json.dumps(polygon["geometry"]["coordinates"]),
                        ),
